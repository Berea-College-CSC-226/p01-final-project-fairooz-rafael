...
class ShoppingApp:
    ...
    def add_to_cart(self):
        ...
        # refresh product list to visually reflect nothing changes yet
    ...
    #             item["quantity"] += 1  #careful here
    #             #product.update_stock(1)  #shouldn't update yet, only after confirming the stock !
    ...
    #             return
    #
    #     self.cart.append({"product": product, "quantity": 1}) #esta vaina si esta bien
    ...
    def build_cart_page(self):
        ...
        if not self.cart:
            tk.Label(frame, text="(Cart is empty)", fg="white", bg="#2c2c2c", font=("Arial", 14)).pack(pady=10)
            self.app_button(self.root, "Back", self.build_customer_main).pack(pady=5)
            return

        counter = Counter(self.cart)

        row = 0
        total_price = 0

        for product, qty in counter.items():
            tk.Label(frame, text=f"{product.product_name} - ${product.selling_price:.2f}",
                     bg="#2c2c2c", fg="white", font=("Arial", 12),
                     anchor="w", width=40).grid(row=row, column=0, pady=5)

            tk.Label(frame, text=f"x{qty}", bg="#2c2c2c", fg="white",
                     font=("Arial", 12)).grid(row=row, column=1)

            total_price += product.selling_price * qty
            row += 1

        tk.Label(self.root, text=f"Total: ${total_price:.2f}", font=("Arial", 14),
                 fg="white", bg="#1c1c1c").pack(pady=10)

        self.app_button(self.root, "Confirm Purchase", lambda: self.checkout(total_price)).pack(pady=5)
        self.app_button(self.root, "Back", self.build_customer_main).pack(pady=5)
    ...
    def checkout(self, total):
        ...
        for p in self.cart:
            p.stock -= 1  # only now stock changes
