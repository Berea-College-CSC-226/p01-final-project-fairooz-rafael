...
def is_valid_input(barcode):
    ...
    :param barcode: the bar code we want to check
...
def is_valid_modulo(barcode):
    ...
    :param barcode: The bar code in the form of a string we want to check
    ...
    n = (3*odds + evens)%10
    #print(n)
    if (10-n)==int(barcode[-1]):
        return True
    elif n==barcode[-1]:
        return True
    else:
        return False
def translate(barcode_num, pos):
    """
...
def mid(t, length):
    ...
    black_line(t, 200)
    white_line(t, 200)
def draw_code(barcode, t):
    """
    Function to draw the barcode
    :param barcode: the complete barcode we want to draw
    :param t: a turtle object
    :return: none
    """
    t.goto(-275, 125)
